...
#' }
simple_basis_search_popa <- function(po.formula, pa.formula, po.data, pa.data, po.fold.id, pa.fold.id, max.basis.functions, coord.names = c("x", "y"), quad.weights.name = "quad.size", radius.type = c("diag", "limiting"), bf.matrix.type = c("sparse", "dense"), domain.data, approx.with = c("laplace", "variational")) {
