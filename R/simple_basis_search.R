...
#' }
simple_basis_search <- function(scampr.model, po.fold.id, pa.fold.id, max.basis.functions, radius.type = c("diag", "limiting"), domain.data, loglik.type = c("laplace", "variational", "both")) {
  ...
  result <- switch(switch.id,
                   ...
                   pa.laplace = simple_basis_search_pa(pa.formula = scampr.model$formula, pa.data = scampr.model$data, pa.fold.id, max.basis.functions = max.basis.functions, coord.names = scampr.model$coord.names, radius.type = radius.type, bf.matrix.type = "sparse", domain.data, approx.with = "laplace"),
