  ...
  case laplace:
  {
    ...
    case sparse:
    {
      ...
      switch (random_bias_type){
      case field1:
      {
        // Parameters specific to IDM accounting for bias with a secondary latent field
        PARAMETER_VECTOR(random_bias); // Biasing coefficients (are random in this instance)
        PARAMETER_VECTOR(log_variance_component_bias); // log(Standard Dev.) of random coefficients within spatial res., l

        vector<Type> Zrandom_bias_pres = Z_PO_pres * random_bias; // random biasing field at presence pts.
        vector<Type> Zrandom_bias_quad = Z_PO_quad * random_bias; // random biasing field at quadrature pts.
        ...
        for (int l = 0; l < bf_per_res.size(); ++l) {                       // loop through basis function resolutions
          for (int k = 0; k < bf_per_res(l); ++k) {                         // loop through random coefficients within resolution l
            LL_random += dnorm(random_bias(k + LCV), mu, PriorSD_bias(l), true); // add the normal pdf contribution to the log-likelihood
          }
          LCV = LCV + bf_per_res(l);                                        // adjust the loop-control-variable so that next set of K_l coefficients are used
        }
      ...
      case field2:
      {
        ...
        vector<Type> Zrandom_bias_pres = Z2_PO_pres * random_bias; // random biasing field at presence pts.
        vector<Type> Zrandom_bias_quad = Z2_PO_quad * random_bias; // random biasing field at quadrature pts.
    ...
    case dense:
    {
      ...
      } // end of random_bias_type switch
